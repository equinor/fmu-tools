...
APPLICATIONROOT := fmu
APPLICATION := fmu/tools
APPLICATIONPKG := fmu_tools
SRCAPPLICATION := src/fmu/tools
...
docsrun: clean ## generate Sphinx HTML documentation, including API docs
	...
	rm -fr docs/_build
	sphinx-apidoc -H "API for fmu.tools" -o docs ${TOPSRCAPPLICATION}
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
siteinstall: dist ## Install in /project/res (Trondheim) using $TARGET
	...
	@echo "Install run scripts..."
	$(foreach RUNAPP, ${RUNAPPS}, rsync -ca bin/${RUNAPP} ${BININSTALL}/.; )
