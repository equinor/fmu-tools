...
def read_specification_file(config_file_name, check=True):
...
def check_specification(spec_dict):
...
def define_new_variable_names_and_correlation_matrix(
    orig_var_names, facies_name, new_var_names, orig_corr_matrix
):
...
def sort_new_var_names(original_variable_names, facies_name, new_variable_names):
    sorted_keep_list = []
...
def set_new_var_name(facies_name, petro_name):
...
def create_new_petro_job_per_facies(spec_dict, debug_print=False, report_unused=False):
