...
def test_combine_selectors():
    ...
    selections["ZONE"] = [["Ile", "Tofte"], ["Aare2.13"], ["Aare2.12"], ["all"]]

    selections["REGION"] = [["oil_zone_Ile_Tofte"], ["gas_zone_P1"], ["all"]]

    comb = find_combinations(selections)

    # Check correct number of combinations 1*4*3 and result of (1,1,1)
    assert len(comb) == 12
    assert comb[0] == [["iter-0"], ["Ile", "Tofte"], ["oil_zone_Ile_Tofte"]]
...
def test_calc_tornadoinput():
    ...
    (tornadotable, ref_value) = calc_tornadoinput(
        ...
        "STOIIP_OIL",
        ["ITER", "ZONE", "REGION"],
        [["iter-0"], ["Ile", "Tofte"], ["oil_zone_Ile_Tofte"]],
        "rms_seed",
        "percentage",
    )
